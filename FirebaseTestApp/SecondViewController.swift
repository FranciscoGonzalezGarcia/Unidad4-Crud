...
class SecondViewController: UIViewController {
    ...
    func getData() {
        FirebaseReference.child("CrudDos").observe(.childAdded, with:  { (snapshot) in
    ...
    func removeObjectFromFirebase(model: ModelTwo) {
        FirebaseReference.child("CrudDos").child(model.id).removeValue()
    }
    
    func updateData() {
        if nameField.text != "" && reusableText.text != "" {
            FirebaseReference.child("CrudDos").child(selectedModel.id).setValue(["NameTwo": nameField.text, "ReusableValue": reusableText.text])
            models[selectedIndex].name = nameField.text ?? ""
            models[selectedIndex].reusableValue = reusableText.text ?? ""
            nameField.text = ""
            reusableText.text = ""
            isEditingData = false
    ...
    @IBAction func posData(_ sender: Any) {
        if isEditingData {
            ...
            if nameField.text != "" && reusableText.text != "" {
                FirebaseReference.child("CrudDos").childByAutoId().setValue(["NameTwo": nameField.text, "ReusableValue": reusableText.text])
                nameField.text = ""
                reusableText.text = ""
            } else {
                print("Missing fields")
...
extension SecondViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        reusableText.text = selectedModel.reusableValue
    }
