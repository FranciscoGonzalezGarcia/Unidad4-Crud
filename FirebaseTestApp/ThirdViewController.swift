...
class ThirdViewController: UIViewController {
    ...
    func getData() {
        FirebaseReference.child("CrudTres").observe(.childAdded, with:  { (snapshot) in
    ...
    func removeObjectFromFirebase(model: ModelThird) {
        FirebaseReference.child("CrudTres").child(model.id).removeValue()
    }
    
    func updateData() {
        if nameField.text != "" && clasificationField.text != "" && typeField.text != "" {
            FirebaseReference.child("CrudTres").child(selectedModel.id).setValue(["Name": nameField.text, "ClasificationOf": clasificationField.text, "TypeOfData": typeField.text])
    ...
    @IBAction func posData(_ sender: Any) {
        if isEditingData {
            ...
            if nameField.text != "" && clasificationField.text != "" && typeField.text != "" {
                FirebaseReference.child("CrudTres").childByAutoId().setValue(["Name": nameField.text, "ClasificationOf": clasificationField.text, "TypeOfData": typeField.text])
                nameField.text = ""
                clasificationField.text = ""
                typeField.text = ""
            } else {
